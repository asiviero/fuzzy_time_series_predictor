...
def fuzzify_historical_data(data,base_series,partition_size,umin,order,ignore_secondary = True):  
  ...
  #print "Comparing to : " + str(input_time_series['Fuzzy Class'][-order:])
  x1 = input_time_series['Fuzzy Class'][-order:].values;
  
  historical_fuzzified_data = [];
  for i in range(len(input_time_series[base_series])-order-1):
    _rel_lhs_classes = input_time_series['Fuzzy Class'][i:i+order].values;
    _dist = leu_euclidean(x1,
                          _rel_lhs_classes, not ignore_secondary, not ignore_secondary);
...
def leu_predictor(data,base_series,partition_size = 0.01,scale_factor = 1,nPredictions=1,k_limit = 5,order=3):
  ...
  tested_series = [x for x in data.columns if x != base_series];  
    ...
    #corr_matrix = np.corrcoef(d_data,rowvar=0)    
...
#     print dict(zip(tested_series,eigvec.T[:1][0][1:]))
    #pca_indexes = dict(zip(tested_series,eigvec.T[:1][0][1:]));
  ...
  for i in range(nPredictions):
    ...
    for i in range(k_limit) :
      midpoint_vector.append(midpoint_fuzzy_class(sorted_historical_fuzzified_data[i]['RHS'],partition_size, umin))
      euclidean_distance_vector.append(sorted_historical_fuzzified_data[i]['ED'])
...
if __name__ == "__main__":
  ...
  # Read data
  d_data = pd.read_csv('asian_indexes.csv', dayfirst = True, index_col = 'DATE', parse_dates = True);
  d_data = d_data * 1e-5;
